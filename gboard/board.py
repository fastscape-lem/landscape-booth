...
class Board:
    ...
    def __init__(self, scale=3, buckets_height=0):
        self.scale = scale
    ...
    def setup_play_widgets(self):
        self.play_widgets = {
            ...
            'reset_particle': Button(description="Reset particle", disabled=True),
    ...
    def set_erosion_params(self):
        u = self.toposim_widgets['u'].value*self.get_uplift_pattern_from_camera()
    ...
    def image_to_uplift_rate(self, im):
        ...
        temp = im * 0.1 + filters.sobel(im) * 5e18
        frangi = filters.frangi(temp)
        u_rate = (np.abs(im - 3.5e2) * 5e-3 + frangi * 1.5 + sobel * 8e13) * 4e-4
        u_rate = np.where(temp > 8, u_rate, 0)
        
        u_rate = 1e-3*(u_rate - np.nanmin(u_rate))/(np.nanmax(u_rate) - np.nanmin(u_rate))
        
        return u_rate

    def get_uplift_pattern_from_camera(self):
        im = self.image_reader.get_next_data()
        im = PIL.Image.fromarray(im)
        
        width, height = im.size   # Get dimensions

        new_width = self.toposim.shape[0]*2
        new_height = self.toposim.shape[1]*2
        ...
        im = block_reduce(im, (2, 2, 1), func=np.mean).astype(np.uint8)
        
        # remove background - very slow
        #im = rembg.remove(im)
        ...
        if self.last_im is None:
            u_rate = self.image_to_uplift_rate(im)
        else:
            u_rate = self.image_to_uplift_rate(im) + 10*(self.image_to_uplift_rate(im) - self.image_to_uplift_rate(self.last_im))
        u_rate[0, :] = 0
        u_rate[-1, :] = 0
        u_rate[:, 0] = 0
        u_rate[:, -1] = 0
    ...
    def start(self, b):
        ...
        self._running = True
        self.image_reader = imageio.get_reader('<video{}>'.format(self.camera_id), fps=15)
        self.process.start()
